...
import { Textarea } from "@/components/ui/textarea"; // Adding Textarea for potential use in notes/comments
import {
  Search,
...
const SupportUsers = () => {
  ...
  const getRoleColor = (role: string) => {
    switch (role) {
      ...
      case 'Student': return 'bg-teal-500';
      case 'Institute Admin': return 'bg-yellow-600';
      default: return 'bg-gray-500';
    }
  };
  ...
  const handleCreateUser = () => {
    if (!newUser.name || !newUser.email || !newUser.phone || !newUser.institute || !newUser.location) {
        ...
        return (
          <>
            ...
            <div className="space-y-4 text-sm">
              ...
              <div><span className="font-semibold">Institute:</span> {selectedUser.institute}</div>
              ...
              <div><span className="font-semibold">Joined Date:</span> {selectedUser.joinedDate}</div>
              <div><span className="font-semibold">Last Login:</span> {selectedUser.lastLogin}</div>
              <div><span className="font-semibold">Total Tickets:</span> {selectedUser.totalTickets}</div>
              <div><span className="font-semibold">Open Tickets:</span> {selectedUser.openTickets}</div>
              <div><span className="font-semibold">Account Issues:</span> {selectedUser.accountIssues}</div>
              <div><span className="font-semibold">Payment Issues:</span> {selectedUser.paymentIssues}</div>
            </div>
          </>
        );
      case 'reset-password':
  ...
  const handleExportUsers = () => {
    ...
    const headers = ['ID', 'Name', 'Email', 'Phone', 'Location', 'Institute', 'Role', 'Status', 'Joined Date', 'Last Login', 'Total Tickets', 'Open Tickets', 'Account Issues', 'Payment Issues'];
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <CardDescription>Support for parents, students, teachers, and administrators</CardDescription>
        </div>
        <div className="flex items-center space-x-4">
          <Button variant="outline" className="flex items-center" onClick={handleExportUsers}>
            ...
            <DialogContent>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input id="name" value={newUser.name} onChange={(e) => handleNewUserChange('name', e.target.value)} placeholder="e.g., Jane Doe" />
                </div>
                <div>
                  <label className="text-sm font-medium">Email</label>
                  <Input id="email" type="email" value={newUser.email} onChange={(e) => handleNewUserChange('email', e.target.value)} placeholder="e.g., jane.doe@example.com" />
                </div>
                <div>
                  <label className="text-sm font-medium">Phone</label>
                  <Input id="phone" value={newUser.phone} onChange={(e) => handleNewUserChange('phone', e.target.value)} placeholder="e.g., +91 9876543210" />
                </div>
                <div>
                  <label className="text-sm font-medium">Location</label>
                  <Input id="location" value={newUser.location} onChange={(e) => handleNewUserChange('location', e.target.value)} placeholder="e.g., Mumbai, Maharashtra" />
                </div>
                <div>
                  <label className="text-sm font-medium">Institute</label>
                  <Input id="institute" value={newUser.institute} onChange={(e) => handleNewUserChange('institute', e.target.value)} placeholder="e.g., ABC International School" />
                </div>
                <div>
                ...
                <div className="lg:col-span-2 space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center mt-4">
                    <Card className="p-3 space-y-1">
                       <div className="text-xl font-bold text-blue-600">{user.totalTickets}</div>
                       <div className="text-sm text-muted-foreground">Total Tickets</div>
                    </Card>
                    <Card className="p-3 space-y-1">
                       <div className="text-xl font-bold text-orange-600">{user.openTickets}</div>
                       <div className="text-sm text-muted-foreground">Open Tickets</div>
                    </Card>
                    <Card className="p-3 space-y-1">
                       <div className="text-xl font-bold text-red-600">{user.accountIssues}</div>
                       <div className="text-sm text-muted-foreground">Account Issues</div>
                    </Card>
                    <Card className="p-3 space-y-1">
                       <div className="text-xl font-bold text-yellow-600">{user.paymentIssues}</div>
