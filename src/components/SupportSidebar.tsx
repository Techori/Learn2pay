...
const SupportSidebar = ({ isCollapsed, onToggle, activeTab, setActiveTab }: SupportSidebarProps) => {
  const menuItems = [
    { icon: BarChart3, label: 'Dashboard', value: 'dashboard' },
    { icon: Ticket, label: 'Tickets', value: 'tickets' },
    { icon: Building, label: 'Institutes', value: 'institutes' },
        ...
        {!isCollapsed && (
          <div className="mt-4">
            <select className="w-full bg-gray-700 text-white p-2 rounded text-sm border border-gray-600 focus:ring focus:ring-blue-500 focus:border-blue-500">
              <option>Support Dashboard</option>
              {/* Add other relevant options if needed */}
            </select>
            <div className="text-xs text-gray-400 mt-1">Support ID: SUP2024001</div>
          </div>
        )}
      </div>
