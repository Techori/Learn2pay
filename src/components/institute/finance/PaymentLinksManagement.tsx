...
const PaymentLinksManagement = () => {
  ...
  const paymentLinkSummary = [
    {
      icon: Link,
      title: "Active Links",
      value: paymentLinks.filter(link => link.status === "Active").length.toString(),
      color: "text-blue-400",
    },
    {
      icon: IndianRupee,
      title: "Total Collections",
      value: `₹${paymentLinks.reduce((sum, link) => sum + parseFloat(link.amount.replace('₹', '').replace('L', '')), 0).toLocaleString()}`,
      color: "text-green-400",
    },
    {
      icon: Users,
      title: "Students Reached",
      value: paymentLinks.reduce((sum, link) => sum + link.payments, 0).toLocaleString(),
      color: "text-purple-400",
    },
    {
      icon: CheckCircle,
      title: "Success Rate",
      value: `${(paymentLinks.reduce((sum, link) => sum + link.payments, 0) / (paymentLinks.reduce((sum, link) => sum + link.clicks, 0) || 1) * 100).toFixed(1)}%`,
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-700">
              <thead className="bg-gray-900">
                <tr>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Link Name</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Amount</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Expiry Date</th>
              ...
              <tbody className="divide-y divide-gray-800">
                {paymentLinks
                  .filter(link =>
                    ...
                    link.linkUrl.toLowerCase().includes(searchQuery.toLowerCase())
                  )
                  .filter(link => {
                  ...
                  <tr key={index} className="hover:bg-gray-800/70">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-white">{link.linkName}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center space-x-2">
                        <Button variant="ghost" className="text-gray-400 hover:text-orange-500" onClick={() => handleViewLink(link)}>
                          <Eye className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" className="text-gray-400 hover:text-orange-500" onClick={() => handleCopyLink(link.linkUrl)}>
                          <Copy className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" className="text-gray-400 hover:text-orange-500" onClick={() => handleShareLink(link.linkName, link.linkUrl)}>
      ...
      {selectedLink && (
        <Dialog open={isViewLinkDialogOpen} onOpenChange={setIsViewLinkDialogOpen}>
          <DialogContent className="max-w-xl bg-gray-800 text-white p-6 rounded-lg">
            <DialogHeader>
              <DialogTitle className="text-2xl font-bold text-white">Payment Link Details</DialogTitle>
              <DialogDescription className="text-gray-400">Viewing details for {selectedLink.linkName}</DialogDescription>
            </DialogHeader>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
              <div>
                <p className="text-sm text-gray-400">Link Name:</p>
                <p className="text-white font-medium">{selectedLink.linkName}</p>
              </div>
              <div>
                <p className="text-sm text-gray-400">Link URL:</p>
                <p className="text-white font-medium break-all">{selectedLink.linkUrl}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <Badge className={selectedLink.status === "Active" ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}>
                  {selectedLink.status}
                </Badge>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-white font-medium">{selectedLink.payments}</p>
              </div>
              <div>
                <p className="text-sm text-gray-400">Conversion:</p>
                <p className="text-white font-medium">{selectedLink.conversion}</p>
              </div>
            </div>
            <DialogFooter>
              <Button onClick={() => setIsViewLinkDialogOpen(false)}>Close</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      )}
