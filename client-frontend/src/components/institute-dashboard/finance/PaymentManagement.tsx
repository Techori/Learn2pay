...
const PaymentManagement = () => {
  ...
  const handleSaveEditedPayment = () => {
    if (selectedPayment) {
      setPaymentTransactions(paymentTransactions.map(pt => pt.transactionId === selectedPayment.transactionId ? selectedPayment : pt));
  ...
  const handleEditInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedPayment) {
      setSelectedPayment(prev => {
        if (!prev) return null;
        return { ...prev, [id.replace('editPayment', '').toLowerCase()]: value };
      });
    }
  };
    ...
    <div className="space-y-6">
            ...
            <table className="min-w-full divide-y divide-gray-700">
              ...
              <tbody className="divide-y divide-gray-800">
                {paymentTransactions
                  ...
                  .filter(transaction => {
                    if (filters.date && transaction.paymentDate !== filters.date) {
                      return false;
                    }
                  ...
                  <tr key={index} className="hover:bg-gray-800/70">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center space-x-2">
                        <Button variant="ghost" size="sm" className="text-gray-400 hover:text-orange-500" onClick={() => handleViewPayment(transaction)}>
                          <Eye className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="sm" className="text-gray-400 hover:text-orange-500" onClick={() => handleEditPayment(transaction)}>
        ...
        <Dialog open={isViewPaymentDialogOpen} onOpenChange={setIsViewPaymentDialogOpen}>
          <DialogContent className="max-w-xl bg-gray-800 text-white p-6 rounded-lg">
            <DialogHeader>
              ...
              <DialogDescription className="text-gray-400">Viewing details for transaction ID: {selectedPayment.transactionId}</DialogDescription>
            </DialogHeader>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
              ...
              <div>
                ...
                <p className="text-white font-medium">{selectedPayment.studentId}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-white font-medium">{selectedPayment.paymentDate}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-white font-medium">{selectedPayment.transactionId}</p>
              </div>
              <div>
                <p className="text-sm text-gray-400">Status:</p>
                <Badge className={selectedPayment.status === "Completed" ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}>
                  {selectedPayment.status}
                </Badge>
              </div>
            </div>
            <DialogFooter>
              <Button onClick={() => setIsViewPaymentDialogOpen(false)}>Close</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      )}
      ...
      {selectedPayment && (
        <Dialog open={isEditPaymentDialogOpen} onOpenChange={setIsEditPaymentDialogOpen}>
          <DialogContent className="max-w-xl bg-gray-800 text-white p-6 rounded-lg">
            <DialogHeader>
              <DialogTitle className="text-2xl font-bold text-white">Edit Payment</DialogTitle>
              <DialogDescription className="text-gray-400">Editing details for transaction ID: {selectedPayment.transactionId}</DialogDescription>
            </DialogHeader>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
              <Input id="editPaymentStudentName" type="text" placeholder="Student Name" className="bg-gray-700 border-gray-600 text-white placeholder-gray-400" value={selectedPayment.studentName} onChange={handleEditInputChange} />
              <Input id="editPaymentStudentId" type="text" placeholder="Student ID" className="bg-gray-700 border-gray-600 text-white placeholder-gray-400" value={selectedPayment.studentId} onChange={handleEditInputChange} />
              <Input id="editPaymentAmount" type="text" placeholder="Amount" className="bg-gray-700 border-gray-600 text-white placeholder-gray-400" value={selectedPayment.amount} onChange={handleEditInputChange} />
              <Input id="editPaymentPaymentDate" type="text" placeholder="Payment Date" className="bg-gray-700 border-gray-600 text-white placeholder-gray-400" value={selectedPayment.paymentDate} onChange={handleEditInputChange} />
              <Input id="editPaymentPaymentMethod" type="text" placeholder="Payment Method" className="bg-gray-700 border-gray-600 text-white placeholder-gray-400" value={selectedPayment.paymentMethod} onChange={handleEditInputChange} />
              <Input id="editPaymentTransactionId" type="text" placeholder="Transaction ID" className="bg-gray-700 border-gray-600 text-white placeholder-gray-400" value={selectedPayment.transactionId} onChange={handleEditInputChange} />
              <Input id="editPaymentStatus" type="text" placeholder="Status" className="bg-gray-700 border-gray-600 text-white placeholder-gray-400" value={selectedPayment.status} onChange={handleEditInputChange} />
            </div>
            <DialogFooter>
